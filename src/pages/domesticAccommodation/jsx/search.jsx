...
import Search_where from "./search_where.jsx";
import People from "./people.jsx";
import SearchCalendar from "./search_calendar.jsx";
...
function Search() {
  ...
  const [peopleCount, setPeopleCount] = useState({ adults: 1, children: 0 });
  const peopleRef = useRef();

  function searchWhenOn() {
    searchWhereRef.current.hidden = false;
  }
  ...
  function handlePeopleChange(newCount) {
    setPeopleCount(newCount);
  }

  function formatPeopleDisplay() {
    const total = peopleCount.adults + peopleCount.children;
    if (peopleCount.children === 0) {
      return `성인 ${peopleCount.adults}명`;
    }
    return `성인 ${peopleCount.adults}명 · 아동 ${peopleCount.children}명`;
  }

  useEffect(() => {
    function handleOutside(e) {
      if (searchWhereRef.current && !searchWhereRef.current.contains(e.target)) {
        searchWhereRef.current.hidden = true;
      }
  ...
  }, [searchWhereRef, calendarRef, peopleRef]);

  return (
    <div>
      <div className="info">
        <div className="info_div" onClick={searchWhenOn}>
          <svg
            width="20"
            height="20"
            viewBox="0 0 24 24"
          ...
          <span className="search_span" id="search_span">
            어디로 떠나시나요?
          </span>
        </div>

        <div ref={searchWhereRef} hidden={true} id="Search_where">
          {<Search_where />}
        </div>

        <div className="info_div" onClick={() => setShowCalendar(true)}>
          <svg
            width="20"
            height="20"
            viewBox="0 0 24 24"
          ...
          <span className={`search_span ${dateRange.startDate ? 'selected' : ''}`}>
            {formatDisplayDate()}
          </span>
        </div>

        <div ref={calendarRef} className="calendar-wrapper">
          {showCalendar && <SearchCalendar onDateSelect={handleDateSelect} />}
        </div>

        <div className="info_div" onClick={() => setPeople(true)}>
          <svg
            width="20"
            height="20"
            viewBox="0 0 24 24"
          ...
          <span className="search_span_people">{formatPeopleDisplay()}</span>
        </div>

        <div ref={peopleRef} className="people-wrapper">
          {people && <People onPeopleChange={handlePeopleChange} />}
        </div>
